...
class Simulador_Test {
  ...
  val piccolo : Guerrero = new Guerrero("Piccolo", esferasDelDragon, 200, 500, Namekusein, Luchando, todosSaben++List(Fusion(krilin), Magia(dejarInconsciente), Onda(40)))
  val majinBuu: Guerrero = new Guerrero("Majin Buu", List(Arma(Filosa)), 300, 700, Monstruo({case (a, o) => {a.tusMovimientos(o.movimientos)} }), Luchando, todosSaben++List(UsarItem(Arma(Filosa)), ComerseAlOponente))
  ...
  def krilinSeDejaFajarTest() = {
    ...
    assertEquals(k, krilin)
    assertEquals(p, piccolo)
  }
